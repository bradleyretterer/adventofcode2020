...
groups.forEach(group => {
    ...
    const personInGroup = group.split("\n");

    personInGroup.forEach(person => {
        for (let question of person) {
            const currentCount = charToCount.get(question);

            if (currentCount === undefined) {
                charToCount.set(question, 1);
            } else {
                charToCount.set(question, currentCount + 1);
            }
        }
    });

    const numberOfPeopleInGroup = personInGroup.length;
    let questionsEveryoneInGroupAnswered = 0;
    for (const count of charToCount.values()) {
        if (count === numberOfPeopleInGroup) {
            questionsEveryoneInGroupAnswered++;
        }
    }

    totalSum += questionsEveryoneInGroupAnswered;
});
