...
passports.forEach(passport => {
    const map = new Map();
    const fieldsWithPossibleNewLines = passport.split(" ");

    fieldsWithPossibleNewLines.forEach(fieldWithPossibleNewLine => {
        const splitFieldsByNewLine = fieldWithPossibleNewLine.split("\n");

        splitFieldsByNewLine.forEach(fieldAndValue => {
...
function isValidPassport(map) {
    return REQUIRED_FIELDS.every(requiredField => {
        const value = map.get(requiredField);

        switch (requiredField) {
            case "byr": return isValidBirthYear(value);
            case "iyr": return isValidIssueYear(value);
            case "eyr": return isValidExpirationYear(value);
...
function isValidIssueYear(value) {
    if (!value) {
        return false;
    }
...
function isValidExpirationYear(value) {
    if (!value) {
        return false;
    }
...
function isValidHeight(value) {
    const groups = /(\d+)(cm|in)/.exec(value);

    if (groups === null) {
        return false;
    }

    const [_, numberString, units] = groups;
...
function isValidEyeColor(value) {
    return VALID_EYE_COLORS.has(value);
}
