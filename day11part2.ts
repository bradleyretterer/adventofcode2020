const input = `LLLLL.L.LLLLLLL.LL.LLLLL.LLLLLLL..LLLLL.LLLLL.L..LLLLL..LLLLLLLL.LLLL.LLL.LLLLLLL.LLLLLLLLLL
...
const seats = makeArrayFromSeats(input);
let newSeats = makeSeatArrangements(seats);
let numberOfSeatChanges = calculateNumberOfSeatChanges(seats, newSeats);

while (numberOfSeatChanges !== 0) {
    const oldSeats = newSeats;

    newSeats = makeSeatArrangements(oldSeats);
    numberOfSeatChanges = calculateNumberOfSeatChanges(oldSeats, newSeats);
}

console.log(numberOfSeatsOccupied(newSeats));

function makeArrayFromSeats(seatsAsString) {
    const rowsOfSeats = seatsAsString.split("\n");

    return rowsOfSeats.map(row => row.split(""));
}

function makeSeatArrangements(seats) {
    const newSeats = [];

    for (let i = 0; i < seats.length; i++) {
        for (let j = 0; j < seats[i].length; j++) {
            const newSeat = getNewSeat(i, j, seats);
            const newRow = newSeats[i];

            if (!newRow) {
                newSeats[i] = [];
            }

            newSeats[i].push(newSeat);
        }
    }
...
function getNewSeat(i, j, seats) {
    const seat = seats[i][j];

    if (seat === EMPTY_SEAT && !hasOccupiedAdjacentSeats(i, j, seats)) {
        return OCCUPIED_SEAT;
    } else if (seat === OCCUPIED_SEAT && numberOfOccupiedAdjacentSeats(i, j, seats) >= 5) {
        return EMPTY_SEAT;
    } else {
        return seat;
    }
...
function numberOfOccupiedAdjacentSeats(i, j, seats) {
    return DIRECTIONS.reduce((occupiedSeatCount, direction) => {
        if (firstSeatInDirectionIsOccupied(direction, i, j, seats)) {
            occupiedSeatCount++;
        }
...
function hasOccupiedAdjacentSeats(i, j, seats) {
    return numberOfOccupiedAdjacentSeats(i, j, seats) !== 0;
}

function firstSeatInDirectionIsOccupied(direction, i, j, seats) {
    const [rowDirection, colDirection] = direction;
    let newI = i + rowDirection;
    let newJ = j + colDirection;

    while (!seatIsOutOfBounds(newI, newJ, seats)) {
        const currentSeat = seats[newI][newJ];

        if (currentSeat === OCCUPIED_SEAT) {
            return true;
        }

        if (currentSeat === EMPTY_SEAT) {
            return false;
        }

        newI = newI + rowDirection;
        newJ = newJ + colDirection;
    }

    return false;
}

function seatIsOutOfBounds(i, j, seats) {
    return i < 0 || j < 0 || i >= seats.length || j >= seats[i].length;
}

function seatIsOccupied(i, j, seats) {
    if (seatIsOutOfBounds(i, j, seats)) {
        return false;
    }

    return seats[i][j] === OCCUPIED_SEAT;
}

function calculateNumberOfSeatChanges(oldSeats, newSeats) {
    let numberOfSeatChanges = 0;
    for (let i = 0; i < oldSeats.length; i++) {
        for (let j = 0; j < oldSeats[i].length; j++) {
            const oldSeat = oldSeats[i][j];
            const newSeat = newSeats[i][j];
...
function numberOfSeatsOccupied(seats) {
    return seats.reduce((totalSeatsOccupied, row) => {
        return totalSeatsOccupied + row.reduce((seatsOccupiedInRow, seat) => {
            if (seat === OCCUPIED_SEAT) {
                seatsOccupiedInRow++;
            }

            return seatsOccupiedInRow;
        }, 0);
    }, 0)
}
