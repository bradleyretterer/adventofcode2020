...
function getNewSeat(i, j, seats) {
    ...
    } else if (seat === OCCUPIED_SEAT && numberOfOccupiedAdjacentSeats(i, j, seats) >= 4) {
        return EMPTY_SEAT;
    } else {
        return seat;
    }
...
function numberOfOccupiedAdjacentSeats(i, j, seats) {
    return DIRECTIONS.reduce((occupiedSeatCount, [row, col]) => {
        if (seatIsOccupied(i + row, j + col, seats)) {
            occupiedSeatCount++;
        }
