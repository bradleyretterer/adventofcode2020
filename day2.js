...
const count = inputArray.reduce((validPasswordCount, input) => {
    const [charRange, charWithColon, password] = input.split(" ");
    const char = charWithColon.charAt(0);
    const [charStart, charEnd] = charRange.split("-");
    let countInPassword = 0;
    ...
    const isValid = countInPassword >= charStart && countInPassword <= charEnd;
    if (isValid) {
        validPasswordCount++;
    }
