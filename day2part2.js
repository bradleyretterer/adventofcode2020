...
const count = inputArray.reduce((validPasswordCount, input) => {
    const [charIndicies, charWithColon, password] = input.split(" ");
    const char = charWithColon.charAt(0);
    const [charIndex1, charIndex2] = charIndicies.split("-");
    
    const charIsAtIndex1 = password.charAt(charIndex1 - 1) === char;
    const charIsAtIndex2 = password.charAt(charIndex2 - 1) === char;
    
    const isValid = (charIsAtIndex1 || charIsAtIndex2) && !(charIsAtIndex1 && charIsAtIndex2);
    if (isValid) {
        validPasswordCount++;
    }
