...
const BAG_SEARCHING_FOR = "shiny gold";
const bagDescriptions = input.split("\n");

const map = new Map();
bagDescriptions.forEach(description => {
    const [_, bagType, containingBags] = /([a-z ]+) bags contain ([a-z0-9 ,]+)/.exec(description);
    const separatedContainingBags = containingBags.split(", ");

    separatedContainingBags.forEach(containingBagString => {
        const [_, numberBags, containingBag] = /(?:(\d+) ([a-z ]+) bags?)|no other bags/.exec(containingBagString);

        if (numberBags === undefined || containingBag === undefined) {
            return;
        }

        const bagTypeInfo = map.get(bagType);
...
let numberOfBagsThatContainOneShinyGoldBag = 0;
